...
try:
	...
	query+=" VALUES('"+user+"' , '"+str(vnc_port)+"', '"+str(proxy_port)+"','"+ipaddr+"','"+os+"')"
	cur.execute(query)
	con.commit()
	
	fn="/tmp/"+user+str(vnc_port)
	osname="os"+user+str(vnc_port)
	cmd="virt-install --name "+osname +"  --ram 1024 --disk path=" + fn + ",size=100  --location=http://127.0.0.1/pub/rhel6 --vcpus=1 --noautoconsole --autostart  --graphics vnc,port="+str(vnc_port)+",listen=0.0.0.0  --force"
	
	li="sshpass -p 00.. ssh -o StrictHostKeyChecking=no root@127.0.0.1 "+cmd
	commands.getoutput(li)
	print "Location:http://127.0.0.1/login/instance.php?os_name="+os
	print
